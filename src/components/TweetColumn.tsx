...
import { useStreamState } from "@/lib/use-stream-state";
...
export function TweetColumn({ onNewTweet }: { onNewTweet: () => void }) {
  ...
  const { status, isRunning, lastError, toggle } = useStreamState();
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
        ...
        {!loading && tweets.length === 0 && isFirebaseConfigured && (
          <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
            {isRunning
              ? "No posts yet. Add an account under Accounts if you haven't."
              : "Paused. Hit Resume to start streaming."}
          </div>
        )}
